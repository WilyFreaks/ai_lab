...
10. **Scenario 1 telemetry model:** keep immediate `R5->R7` directional gap via `telemetry#cnc_interface_counter_json#immediate_gap_*`, and apply slice reroute via `telemetry#cnc_interface_counter_json#reroute_*` keys (`from_slice`, `to_slice`, `pct`, `start_minutes`, `ramp_minutes`).
...
12. **Scenario 1 ThousandEyes timing:** `response_time_ms` supports timed return to baseline through `...back_to_baseline_start_minutes` + `...back_to_baseline_ramp_minutes`.
