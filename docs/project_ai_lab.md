...
17. **Packaging:** on explicit request, promote **`local/data/ui/views/*.xml`** to **`default/data/ui/views/`** with **per-file full copies** (`cp`). Promote **`local/savedsearches.conf`** into **`default/savedsearches.conf`** by **merging** (same idea as **`metadata/local.meta` → `default.meta`**), not by replacing `default` with `local` only unless the user explicitly accepts dropping default-only stanzas. Alert scheduled searches → **`index=alerts`** ship inside **`savedsearches.conf`**. Full workshop reset (`scripts/reset_workshop_state.sh`) runs a **`cp`** for saved searches then merges metadata (see `docs/project_script_design.md` → *reset_workshop_state.sh packaging sync*); verify **`default/savedsearches.conf`** after reset if `local` was not a superset. **`disabled = 1` sync discipline:** if `local/savedsearches.conf` disables a stanza, set the same key in `default/savedsearches.conf` so packaging does not re-enable it on hosts without `local/`.
20. **Workshop MCP config deliverable:** `appserver/static/agent_mcp_json.txt` is the MCP config template for attendees — downloadable via the "MCP Config (JSON)" link on the `scenario_control` dashboard. Attendees save it as `.cursor/mcp.json` (or `.mcp.json`) in their Cursor workspace. Update the IP and token in this file when provisioning a new AMI. The host's own MCP config lives at `.cursor/mcp.json` in this repo (tracked, token is local-only).
...
- **Saved-search-first verification policy:** for app-level checks, prefer saved searches in app `ai_lab`. All 32 current saved searches carry a `description =` field — read it to confirm capability before calling. Key searches by category:
  - **Interface telemetry:** `telemetry_if_counter`, `cnc_interface_ifInPktsRate`, `cnc_interface_ifOutPktsRate`, `cnc_interface_ifInPktsRate_for_a_router` *(parameterized: requires `router="R2"`; currently **disabled=1** — use `cnc_interface_ifInPktsRate` for all-router view)*
  - **SR-TE path:** `cnc_srte_path`
  - **Service health:** `cnc_service_health`
  - **ThousandEyes:** `thousandeyes_response_time_sec`
  - **TWAMP:** `twamp_event_count`, `twamp_dmean`, `twamp_jmean`, `twamp_lostperc`, `twamp_rxpkts`
  - **WDM PM (by-router):** `wdm_FEC_BEF_COR_ER_over_time_by_router`, `wdm_LSBIASCUR_over_time_by_router`, `wdm_SUMOOPCUR_over_time_by_router`, `wdm_SUMIOPCUR_over_time_by_router`, `wdm_BDTEMPCUR_over_time_by_router`, `wdm_EDTMPCUR_over_time_by_router`, `wdm_LOSTOPCUR_over_time_by_router`
  - **WDM PM (transponder-level):** `wdm_FEC_BEF_COR_ER_over_time`, `wdm_LSBIASCUR_over_time`, `wdm_SUMOOPCUR_over_time`, `wdm_SUMIOPCUR_over_time`, `wdm_BDTEMPCUR_over_time`, `wdm_EDTMPCUR_over_time`
  - **Alerts (scheduled):** `Interface Counter Mismatch`, `Packet Loss Threshold Exceeded`, `CNC Service Health Status Degraded`
  - **Forecasting / ML:** `forecast_cdtsm`, `forecast_predict`, `forecast_cdtsm_multi_series`, `forecast_predict_multi_series`
  - Use a recent window (recommended last 5 minutes) when validating active live generation. Baseline data-quality checks run via `scripts/test_baseline.sh` → `scripts/test_backfill.sh`, which include the TWAMP saved searches above.
- **Scenario dashboard XML:** author in `local/data/ui/views/` (Splunk UI saves here); on explicit sync (“copy local dashboard to default”), **full-file** copy per view to `default/data/ui/views/` for Git/AMI. At runtime, **`local` overrides `default`** for the same view name—see `docs/project_dashboard_design.md` (*Scenario dashboard authoring*). **`savedsearches.conf`** uses **merge** into `default/`, not the same one-shot replace as a single dashboard XML file.
