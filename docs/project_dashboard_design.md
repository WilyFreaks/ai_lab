...
- When the user explicitly asks to promote changes (for example “copy local dashboard to default”, “sync `scenario_1_au` from local to default”, or **package workshop UI + searches**), copy **`local/data/ui/views/<view>.xml`** → **`default/data/ui/views/<view>.xml`** as a **full-file replacement** (`cp` per view file; no merge inside the XML). **`savedsearches.conf`** is different: promote with a **`local` → `default` merge** (preserve default-only stanzas), not a blind wholesale replace—see `appserver/static/agent_rules_en.md` §5 and `.cursor/rules/splunk_app_rules.mdc`. **Scheduled alert searches** (saved-search output to **`index=alerts`**) live in **`savedsearches.conf`**, not separate files.
