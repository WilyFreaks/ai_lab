...
print("- disabled=1 sync rule: if a local/ stanza has disabled=1, add the same key to default/ so packaging does not accidentally re-enable the search on hosts without local/.")
print("- btool parity test: after editing a description or key, verify: mv local/savedsearches.conf tmp && splunk btool savedsearches list <stanza> --app=ai_lab | grep <key> ; mv tmp back — both outputs must be identical.")
...
print("- Episode/timeline panel source contract: use index=alerts sourcetype=ai_lab_alert (not legacy index=alert/generic_single_line)")
print("- Regional dashboard lookup contract: scenario_1_au -> router_areas_au.csv, scenario_1_jp -> router_areas_jp.csv")
...
print("- Saved-search description coverage: ALL 32 current saved searches in ai_lab have a description = key (capability first, use case examples at the bottom). Agents can rely on description = being present. Parameterized search: cnc_interface_ifInPktsRate_for_a_router requires router= arg (e.g. router=R2) — NOTE: currently disabled=1 in both local/ and default/; use cnc_interface_ifInPktsRate for all-router view instead.")
print("- Forecasting saved searches (shipped): forecast_cdtsm, forecast_predict (ThousandEyes single-series, 7-day); forecast_cdtsm_multi_series, forecast_predict_multi_series (per-interface traffic all routers, 2-week). Use for ML demo / anomaly context.")
print("- CDTSM xc/xf: xf = fine context (up to 512 points at base resolution); xc = coarse context (up to 512 points at 60x resolution, e.g. 1-min data -> 1-hour xc). Recommended data granularity: 1, 2, 3, or 4 minutes (divide 1440 evenly). Practical max training window: ~40 days (512 xc points × 2-hour coarse = ~1024 hours). See forecast_cdtsm description for full details.")
print("- TWAMP saved searches (app ai_lab): twamp_event_count, twamp_dmean, twamp_jmean, twamp_lostperc, twamp_rxpkts (5m window in SPL; see default/savedsearches.conf)")
