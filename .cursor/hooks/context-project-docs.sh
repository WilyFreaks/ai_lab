...
print("- Scenario alert contract: Interface Counter Mismatch / Packet Loss Threshold Exceeded / CNC Service Health Status Degraded emit to index=alerts sourcetype=ai_lab_alert")
print("- Scenario episode/timeline panels should read index=alerts sourcetype=ai_lab_alert (not legacy index=alert/generic_single_line)")
print("- Scenario regional lookup contract: scenario_1_au uses router_areas_au.csv; scenario_1_jp uses router_areas_jp.csv")
